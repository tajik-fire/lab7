...
public class ClientManager implements Runnable {
    ...
    private final ExecutorService requestReadingThreadPool;
    private final ExecutorService responseSendingThreadPool;
    private volatile boolean running = true;

    public ClientManager(int port, CommandProcessor commandProcessor, AuthService authService, ExecutorService requestReadingThreadPool, ExecutorService responseSendingThreadPool) throws IOException {
        this.serverSocket = new ServerSocket(port);
        this.serverSocket.setSoTimeout(200); // короткий таймаут для accept()
        this.commandProcessor = commandProcessor;
        this.authService = authService;
        this.requestReadingThreadPool = requestReadingThreadPool;
    ...
    public void run() {
        ...
        while (running) {
            try {
                ...
                requestReadingThreadPool.submit(() -> handleClient(clientSocket));
            } catch (SocketTimeoutException e) {
                // Нет клиента — нормально, продолжаем ждать
            } catch (IOException e) {
                if (running) {
    ...
    private void handleClient(Socket clientSocket) {
        ...
        try {
            // Правильный порядок: сначала OutputStream, потом InputStream
            ...
            if (requestObj instanceof Request request) {
                ...
                // --- Authentication/Registration block --- Вход или регистрация обрабатываются отдельно
                if ("register".equals(commandName)) {
                    ...
                    return; // Всегда завершаем обработку после команды register
                    ...
                    return; // Всегда завершаем обработку после команды login
                }
                // --- End of Authentication/Registration block ---


                // --- General command processing for authenticated users ---
                // Для любой другой команды, сначала нам нужно аутентифицировать пользователя
                try {
                    authenticatedUser = authService.authenticate(request.getUsername(), request.getPassword()).orElse(null);
                } catch (SQLException e) {
                ...
                // Если мы дошли сюда, пользователь аутентифицирован и команда не является login/register
                User userForLambda = authenticatedUser;
                final ObjectOutputStream finalOut = out;
                final Socket finalClientSocket = clientSocket;
                requestReadingThreadPool.submit(() -> {
                    try {
                        Response response = commandProcessor.processCommand(request, userForLambda);
                        sendResponse(finalOut, finalClientSocket, response);
                        ...
                        try {
                            sendResponse(finalOut, finalClientSocket, new Response(List.of("Error processing command: " + e.getMessage()), false, 500));
            ...
            // Закрываем сокет здесь, если ошибка произошла до отправки ответа
            try {
                if (in != null) in.close();
